...
static irqreturn_t mxt_irq_handler(int irq, void *_mxt)
{
	...
	mxt->irq_counter++;
	if (mxt->valid_interrupt() && mxt->isPower_On) {
...
static int mxt_power_off(struct i2c_client *client)
{
...
static int mxt_power_on(struct i2c_client *client)
{
	struct mxt_data *mxt = i2c_get_clientdata(client);
	u8 buf[MXT_ID_BLOCK_SIZE];
	int error;
                             ...
                             struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (is_enable){
		mxt_power_on(mxt->client);
	}
	else{
		mxt_power_off(mxt->client);
	}
...
static DEVICE_ATTR(power_state, S_IRUGO | S_IWUSR, power_show, power_store);
			       ...
			       const struct i2c_device_id *id)
{
	...
	if (mxt->init_hw != NULL && mxt->init_hw())
	{
		dev_err(&client->dev, "hardware initialization failed\n");
		goto err_init_hw;
	}
	...
	/* Power Management related initialization */
	mutex_init(&mxt->power_mutex);
	mxt->isPower_On = 1;
	...
	error = device_create_file(&client->dev, &dev_attr_power_state);
	if (error < 0) {
		dev_err(&client->dev, "failed to create the dev_attr_power_state\n");
		goto err_pwr;
	}
...
static int mxt_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	return mxt_power_off(client);
}

static int mxt_resume(struct i2c_client *client)
{
	return mxt_power_on(client);
}
#else
