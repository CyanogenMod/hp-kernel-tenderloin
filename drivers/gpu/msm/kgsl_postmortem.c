...
static int kgsl_dump_yamato(struct kgsl_device *device)
{
...
#if 1
	kgsl_regread(device, REG_LEIA_PC_DEBUG_CNTL, &r1);
	KGSL_LOG_DUMP("REG_LEIA_PC_DEBUG_CNTL: %08X\n", r1);

	kgsl_regread(device, REG_LEIA_PC_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("PC_DEBUG_REG0: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_PC_DEBUG_CNTL, 1);
	kgsl_regread(device, REG_LEIA_PC_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("PC_DEBUG_REG1: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_PC_DEBUG_CNTL, 2);
	kgsl_regread(device, REG_LEIA_PC_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("PC_DEBUG_REG2: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_PC_DEBUG_CNTL, 3);
	kgsl_regread(device, REG_LEIA_PC_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("PC_DEBUG_REG3: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_PC_DEBUG_CNTL, 4);
	kgsl_regread(device, REG_LEIA_PC_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("PC_DEBUG_REG4: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_PC_DEBUG_CNTL, 5);
	kgsl_regread(device, REG_LEIA_PC_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("PC_DEBUG_REG5: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_PC_DEBUG_CNTL, 6);
	kgsl_regread(device, REG_LEIA_PC_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("PC_DEBUG_REG6: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_PC_DEBUG_CNTL, 7);
	kgsl_regread(device, REG_LEIA_PC_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("PC_DEBUG_REG7: %08X\n", r1);


	for( i = 0; i < 2; i++ )
	{
		KGSL_LOG_DUMP("\nRB_DEBUG loop %d\n", i);
		// dump RB DEBUG registers
		kgsl_regwrite(device, REG_LEIA_RB_DEBUG_CNTL, 0);
		kgsl_regread(device, REG_LEIA_RB_DEBUG_DATA+i*0x1000, &r1);
		KGSL_LOG_DUMP("RB_DEBUG_REG00: %08X\n", r1);

		kgsl_regwrite(device, REG_LEIA_RB_DEBUG_CNTL, 1);
		kgsl_regread(device, REG_LEIA_RB_DEBUG_DATA+i*0x1000, &r1);
		KGSL_LOG_DUMP("RB_DEBUG_REG01: %08X\n", r1);

		kgsl_regwrite(device, REG_LEIA_RB_DEBUG_CNTL, 2);
		kgsl_regread(device, REG_LEIA_RB_DEBUG_DATA+i*0x1000, &r1);
		KGSL_LOG_DUMP("RB_DEBUG_REG02: %08X\n", r1);

		kgsl_regwrite(device, REG_LEIA_RB_DEBUG_CNTL, 3);
		kgsl_regread(device, REG_LEIA_RB_DEBUG_DATA+i*0x1000, &r1);
		KGSL_LOG_DUMP("RB_DEBUG_REG03: %08X\n", r1);

		kgsl_regwrite(device, REG_LEIA_RB_DEBUG_CNTL, 4);
		kgsl_regread(device, REG_LEIA_RB_DEBUG_DATA+i*0x1000, &r1);
		KGSL_LOG_DUMP("RB_DEBUG_REG04: %08X\n", r1);

		kgsl_regwrite(device, REG_LEIA_RB_DEBUG_CNTL, 5);
		kgsl_regread(device, REG_LEIA_RB_DEBUG_DATA+i*0x1000, &r1);
		KGSL_LOG_DUMP("RB_DEBUG_REG05: %08X\n", r1);

		kgsl_regwrite(device, REG_LEIA_RB_DEBUG_CNTL, 6);
		kgsl_regread(device, REG_LEIA_RB_DEBUG_DATA+i*0x1000, &r1);
		KGSL_LOG_DUMP("RB_DEBUG_REG06: %08X\n", r1);
	}



	// GRS DEBUG register dump
	kgsl_regwrite(device, REG_LEIA_GRAS_DEBUG_CNTL, 0);
	kgsl_regread(device, REG_LEIA_GRAS_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("GRAS_DEBUG_REG00: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_GRAS_DEBUG_CNTL, 1);
	kgsl_regread(device, REG_LEIA_GRAS_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("GRAS_DEBUG_REG01: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_GRAS_DEBUG_CNTL, 2);
	kgsl_regread(device, REG_LEIA_GRAS_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("GRAS_DEBUG_REG02: %08X\n", r1);

	kgsl_regwrite(device, REG_LEIA_GRAS_DEBUG_CNTL, 3);
	kgsl_regread(device, REG_LEIA_GRAS_DEBUG_DATA, &r1);
	KGSL_LOG_DUMP("GRAS_DEBUG_REG03: %08X\n", r1);
	...
	for( i = 0; i < 2; i++ )
	{
		...
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_CONST_MGR_FSM+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_CONST_MGR_FSM: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_EXP_ALLOC+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_EXP_ALLOC: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_FSM_ALU_0+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_FSM_ALU_0: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_FSM_ALU_1+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_FSM_ALU_1: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_GPR_PIX+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_GPR_PIX: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_GPR_VTX+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_GPR_VTX: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_INPUT_FSM+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_INPUT_FSM: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_MISC+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_MISC: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_MISC_0+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_MISC_0: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_MISC_1+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_MISC_1: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_0+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_PIX_TB_0: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATE_MEM+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_PIX_TB_STATE_MEM: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_0+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_0: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_1+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_1: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_2+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_2: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_3+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_3: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PTR_BUFF+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_PTR_BUFF: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_TB_STATUS_SEL+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_TB_STATUS_SEL: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_TP_FSM+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_TP_FSM: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_VTX_TB_0+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_VTX_TB_0: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_VTX_TB_1+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_VTX_TB_1: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_VTX_TB_STATE_MEM+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_VTX_TB_STATE_MEM: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_VTX_TB_STATE_REG+i*0x1000, &r1);
		KGSL_LOG_DUMP("REG_LEIA_SQ_DEBUG_VTX_TB_STATE_REG: %08X\n", r1);

	}

	// MH_DEBUG register dump
	for( i = 0; i < 64; i++ )
	{
		kgsl_regwrite(device, REG_LEIA_MH_DEBUG_CNTL, i);
		kgsl_regread(device, REG_LEIA_MH_DEBUG_DATA, &r1);
		KGSL_LOG_DUMP("MH_DEBUG_REG%d: %08X\n", i, r1);
	}
	...
	for( i = 0x1b; i <= 0x23; i++ )
	{
		kgsl_regwrite(device, REG_LEIA_RBBM_DEBUG_CNTL,
				0x1600 | i);
		kgsl_regread(device, REG_LEIA_RBBM_DEBUG_OUT, &r1);
		KGSL_LOG_DUMP("MIU 0x%02x: %08x\n", i, r1);

	}


	// CP_STATE register dump
	for( i = 0; i < 0x14; i++ )
	{
		kgsl_regwrite(device, REG_LEIA_CP_STATE_DEBUG_INDEX, i);
		kgsl_regread(device, REG_LEIA_CP_STATE_DEBUG_DATA, &r1);
		KGSL_LOG_DUMP("CP_STATE_DEBUG%d: %08X\n", i, r1);
	}
	...
	for( i = 64; i < 108; i++ )
	{
		...
		//else if(i == 81)
		//{
		//	sprintf(temp, "  <--- Instr pointer");
		//}

		kgsl_regwrite(device, REG_CP_ME_CNTL, i);
		kgsl_regread(device, REG_LEIA_CP_ME_STATUS, &r1);
		//KGSL_LOG_DUMP("CP_ME_STATUS%d: %08X%s\n", i, r1, temp);
		KGSL_LOG_DUMP("CP_ME_STATUS%d: %08X\n", i, r1);
	}
	...
	for( i = 0; i < 16; i++ )
	{
		kgsl_regwrite(device, REG_LEIA_SQ_DEBUG_TB_STATUS_SEL,
				i | (6<<4) | (i<<7) | (1<<11) | (1<<12) | (i<<16) | (6<<20) | (i<<23));
		KGSL_LOG_DUMP("LOOP %d\n", i);


		kgsl_regread(device, REG_LEIA_SQ_DEBUG_VTX_TB_STATE_MEM, &r1);
		KGSL_LOG_DUMP("SQ_DEBUG_VTX_TB_STATE_MEM: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_VTX_TB_STATUS_REG, &r1);
		KGSL_LOG_DUMP("SQ_DEBUG_VTX_TB_STATUS_REG: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATE_MEM, &r1);
		KGSL_LOG_DUMP("SQ_DEBUG_PIX_TB_STATE_MEM: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_0, &r1);
		KGSL_LOG_DUMP("SQ_DEBUG_PIX_TB_STATUS_REG_0: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_1, &r1);
		KGSL_LOG_DUMP("SQ_DEBUG_PIX_TB_STATUS_REG_1: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_2, &r1);
		KGSL_LOG_DUMP("SQ_DEBUG_PIX_TB_STATUS_REG_2: %08X\n", r1);
		kgsl_regread(device, REG_LEIA_SQ_DEBUG_PIX_TB_STATUS_REG_3, &r1);
		KGSL_LOG_DUMP("SQ_DEBUG_PIX_TB_STATUS_REG_3: %08X\n", r1);
	}
