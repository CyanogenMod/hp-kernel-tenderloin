		     ...
		     struct kgsl_context *context)
{
	...
	drawctxt->ctxt_magic_number = KGSL_CONTEXT_INIT_NUMBER;
			  ...
			  struct kgsl_context *context)
{
	...
	if (yamato_device->drawctxt_active == drawctxt) {
		//printk(KERN_DEBUG"In kgsl_drawctxt_destroy() - device->pwrctrl.suspended_ctxt = %x\n"
		//		, device->pwrctrl.suspended_ctxt);

		if(device->pwrctrl.suspended_ctxt == drawctxt)
		{
	...
	drawctxt->ctxt_magic_number = KGSL_CONTEXT_POISON_NUMBER;
	kfree(drawctxt);
			...
			unsigned int flags)
{
	...
	if (drawctxt != NULL) {
		...
		//printk(KERN_DEBUG"kgsl_drawctxt_switch: drawctxt = %x drawctxt->ctxt_magic_number = %x\n"
		//		, drawctxt, drawctxt->ctxt_magic_number);
