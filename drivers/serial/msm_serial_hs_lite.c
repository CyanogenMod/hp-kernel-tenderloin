...
static void msm_hsl_config_port(struct uart_port *port, int flags)
{
	...
	if (msm_serial_hsl_has_gsbi())
		iowrite32(msm_hsl_port->gsbi_ctrl,
	...
	for (i = 0; i < count; i++, s++) {
		if (*s == '\n')	{
			...
			if (num > port->fifosize) {
				msm_hsl_console_putchars(port, port->fifosize,
							 last_break);
...
static int __devinit msm_serial_hsl_probe(struct platform_device *pdev)
{
	...
	if (pdata && pdata->config_gpio) {
		...
		if (pdata->uart_rfr_gpio) {
			ret = gpio_request(pdata->uart_rfr_gpio, "UART_RFR_GPIO");
			if (unlikely(ret)) {
				printk(KERN_ERR "%s: gpio request failed for:"
					"%d\n", __func__, pdata->uart_rfr_gpio);
		...
		if (pdata->uart_cts_gpio) {
			ret = gpio_request(pdata->uart_cts_gpio, "UART_CTS_GPIO");
			if (unlikely(ret)) {
				printk(KERN_ERR "%s: gpio request failed for:"
					"%d\n", __func__, pdata->uart_cts_gpio);
				if (pdata->uart_rfr_gpio)
	...
	/* Default gsbi control value is no flow control, no WRAPPER_CTRL, no CRCI_MUX_CTRL. */
	msm_hsl_port->gsbi_ctrl = GSBI_PROTOCOL_I2C_UART;

	if (pdata && pdata->config_setting) {
		if (pdata->gsbi_protocol == GSBI_PROTOCOL_UART) {
			if (request_rfr && request_cts) {
				msm_hsl_port->gsbi_ctrl &= ~GSBI_PROTOCOL_BMSK;
				msm_hsl_port->gsbi_ctrl |= GSBI_PROTOCOL_UART;
			}
		}

		port->fifosize = pdata->gsbi_fifosize;
	}
