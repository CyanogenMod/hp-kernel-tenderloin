...
int pwm_config(struct pwm_device *pwm, unsigned duty_us, unsigned period_us)
{
	...
	pm8058_pwm_calc_period(period_us, &pwm_conf, 0);

	if(period_us<(1<<22)) {
		// Don't worry overflow.
		pwm_conf.pwm_value = (duty_us*(1<<pwm_conf.pwm_size))/period_us;
	} else {
...
int pwm_config2(struct pwm_device *pwm, unsigned duty_numerator, unsigned duty_denominator, unsigned period_us)
{
	...
	pm8058_pwm_calc_period(period_us, &pwm_conf, 1);
