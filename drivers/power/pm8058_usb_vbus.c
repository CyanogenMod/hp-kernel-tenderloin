...
static int __devinit pm8058_usb_vbus_probe(struct platform_device *pdev)
{
	struct pm8058_chip *pm_chip;
...
static struct platform_driver pm8058_usb_vbus_driver = {
	.probe = pm8058_usb_vbus_probe,
	.remove = __devexit_p(pm8058_usb_vbus_remove),
	.driver = {
		   .name = "pm8058-charger",
		   .owner = THIS_MODULE,
	},
...
static int __init pm8058_usb_vbus_init(void)
{
	return platform_driver_register(&pm8058_usb_vbus_driver);
}

static void __exit pm8058_usb_vbus_exit(void)
{
	platform_driver_unregister(&pm8058_usb_vbus_driver);
}

late_initcall(pm8058_usb_vbus_init);
module_exit(pm8058_usb_vbus_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("PMIC8058 USB VBUS control driver");
MODULE_VERSION("1.0");
MODULE_ALIAS("platform:pm8058_usb_vbus");
