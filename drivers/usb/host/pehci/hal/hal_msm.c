...
isp1763_reg_read16(struct isp1763_dev * dev, u16 reg, u16 data)
{
	reg <<= 1;
#ifdef DATABUS_WIDTH_16
	data = readw_relaxed(dev->baseaddress + ((reg)));
#else
	u8 bval1, bval2;
	bval1 = readb(dev->baseaddress + (reg));
	if (reg == HC_DATA_REG){
...
isp1763_reg_write16(struct isp1763_dev *dev, u16 reg, u16 data)
{
	reg <<= 1;
#ifdef DATABUS_WIDTH_16
	writew_relaxed(data, dev->baseaddress + ((reg)));
#else
	writeb((u8) data, dev->baseaddress + (reg));
	if (reg == HC_DATA_REG){
	...
	u32 end_add, u32 * buffer, u32 length, u16 dir)
{
	...
	u32 w3;
	#ifdef DATABUS_WIDTH_16
	u16 *reg_addr = (u16 *)(dev->baseaddress + (HC_DATA_REG<<1));
	#endif
...
last:
	#ifdef DATABUS_WIDTH_16
	w = (u16)readw_relaxed(reg_addr);
	w2 = (u16)readw_relaxed(reg_addr);
	#else
	...
	while (a > 0) {
		*buffer = w;
		...
		if (a < 16) {
			...
			buffer++;
			#ifdef DATABUS_WIDTH_16
			w3 = (u16)readw_relaxed(reg_addr) |
				(((u16)readw_relaxed(reg_addr))<<16);
			w2 = (u16)readw_relaxed(reg_addr) |
				(((u16)readw_relaxed(reg_addr))<<16);
			w1 = (u16)readw_relaxed(reg_addr) |
				(((u16)readw_relaxed(reg_addr))<<16);
			w  = (u16)readw_relaxed(reg_addr) |
				(((u16)readw_relaxed(reg_addr))<<16);
			#else
			w3 = isp1763_reg_read16(dev, HC_DATA_REG, w) |
				((isp1763_reg_read16(dev, HC_DATA_REG, w))<<16);
			w2 = isp1763_reg_read16(dev, HC_DATA_REG, w) |
				((isp1763_reg_read16(dev, HC_DATA_REG, w))<<16);
			w1 = isp1763_reg_read16(dev, HC_DATA_REG, w) |
				((isp1763_reg_read16(dev, HC_DATA_REG, w))<<16);
			w  = isp1763_reg_read16(dev, HC_DATA_REG, w) |
				((isp1763_reg_read16(dev, HC_DATA_REG, w))<<16);
			#endif
	...
	u32 start_add, u32 end_add, u32 * buffer, u32 length, u16 dir)
{
	...
	if (a == 1) {
	#ifdef DATABUS_WIDTH_16
		writew_relaxed((u16)one, reg_addr);
	#else
	...
	if (a == 2) {
	#ifdef DATABUS_WIDTH_16
		writew_relaxed((u16)two, reg_addr);
	#else
	...
	while (a > 0) {
		#ifdef DATABUS_WIDTH_16
		writew_relaxed((u16)(*buffer), reg_addr);
		#else
		isp1763_reg_write16(dev, HC_DATA_REG, (u16) (*buffer));
		#endif
		if (a >= 3)
		#ifdef DATABUS_WIDTH_16
			writew_relaxed((u16)((*buffer)>>16), reg_addr);
		#else
...
isp1763_module_init(void)
{
	...
	if((result = platform_driver_register(&isp1763_usb_driver)) < 0) {
		pr_err ("%s: driver_register() fail: result :0x%08x\n", __func__, result);
	} else {
		pr_info("%s: driver_register() success: result :0x%08x\n", __func__, result);
	}
...
static int isp1763_hcd_enable(int enabled)
{
	...
	if (enabled && !pdev)
	{
		...
		pdev = platform_device_alloc(hcd_name, 0);
		if (!pdev)
		{
			pr_err("%s: Platform device allocation failed\n", __func__);
			return -ENOMEM;
		}
		if ((ret=platform_device_add_data(pdev, &(isp1763_loc_dev[ISP1763_HC]),
					sizeof((isp1763_loc_dev[ISP1763_HC]))))!=0)
			goto out_free_resources;
		if ((ret = platform_device_add(pdev))!=0)
			goto out_free_resources;
	}
	else if (!enabled && pdev)
	{
		pr_info("%s: platform_device_unregister\n", __func__);
		platform_device_unregister(pdev);
		pdev = NULL;
	}
	pr_info ("%s: on: %d, return\n", __func__, enabled);
	return 0;

out_free_resources:
	pr_err("%s: platform_device_put\n", __func__);
...
isp1763_probe(struct platform_device *pdev)
{
	...
	loc_dev->remove_complete = &remove_complete;
	init_completion(loc_dev->remove_complete);
	pr_info("%s init remove_complete:%p\n", __func__, loc_dev->remove_complete);
...
isp1763_remove(struct platform_device *pdev)
{
	...
	isp1763_hcd_enable(0);

	loc_dev = &isp1763_loc_dev[ISP1763_HC];

	pr_info("%s: wait for remove_complete: %p\n", __func__, loc_dev->remove_complete);
	rc = wait_for_completion_timeout(loc_dev->remove_complete, (5 * HZ));
	if (rc<=0)
	{
		pr_err("%s: Error: HCD Remove is not completed! rc:%d\n", __func__, rc);
	}
	iounmap(loc_dev->baseaddress);
	loc_dev->baseaddress = NULL;
	if (pdata->setup_gpio)
