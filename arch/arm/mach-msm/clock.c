...
/*
 *  Return clk struct by given msm clock id
 */
static struct clk * msm_get_clk_by_id (int id)
{
	struct clk *clk = NULL;
	struct clk *clk_res = NULL;
...
/*
 *  Printk clock info for given clock ID 
 */
void msm_printk_clock_by_id (int id) 
{
	struct clk *c_clk = NULL;
	
	c_clk = msm_get_clk_by_id (id);
	if (c_clk == NULL)
		return;

	printk( KERN_INFO "%16s: %3d: %10lu %d\n",
		c_clk->dbg_name, c_clk->id,
		clk_get_rate(c_clk), c_clk->ops->is_enabled(c_clk->id));
}
 ...
 * clock count.
 */
void __init msm_clock_fixup( uint32_t *fixup_clk_ids, uint32_t fixup_clk_num)
{
    int i;
    struct clk * clk;
    ...
    for(i = 0; i < fixup_clk_num; i++) {
		...
		clk = msm_get_clk_by_id (fixup_clk_ids[i]);

		if(clk && clk->remote_id == 0) {
			clk->flags = 0;
